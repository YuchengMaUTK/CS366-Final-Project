...
class TwitterCrawler(object):
    def __init__(self, since_id=None, clean_dir=False, query=''):
    ...
    def _refine_results(self):
        ...
        for tweet in refined_tweets:
            if self.working_threads < self.MAX_THREADS:
                ...
                t.start()
                self.working_threads += 1
                self.log_info(" Finished tweets: {:<5d} | Total tweets: {:<5d}".format(
                              self.finished_threads, total_tweets))
            time.sleep(0.2)
    ...
    def run(self):
        max_tweet_id = -1
        self.log_info("Twitter key words: %s" % (self.QUERY))
        if self.since_id:
            self.log_info("Start updating tweets since id: {:}...".format(self.since_id))
        else:
            self.log_info("Downloading tweets...")
...
if __name__ == '__main__':
    crawler = TwitterCrawler(clean_dir=True, since_id=None, query='web vulnerabilities')
    crawler.run()
